...
PULP_CFLAGS = -O3 -g -I$(DIR) -DNUM_CORES=${CORES} -DN_SEG=${n_seg} -DNFFT_SEG=${nfft_seg} -DNFFT=${nfft} -DIN_DYN=${in_dyn} -DWINDOW_DYN=${w_dyn} -DBUILD_LUT -DPROFILING # -DPRINT # -DPRINTF_UART #definition and includle

#PWELCH DEFINE
PULP_CFLAGS += -DWIND -DFFT -DPowerDS -DPROF_SWAP #-DPROF_PWELCH -DWINDOWING_CHECK -DINPUT_CHECK
